    ...
    if scene_num and scene_text and active_story_id:
        try:
            ...
            if image_data:
                ...
                text_overlays = None
                if get_template(illustrator.template).visual_narrative:
                    text_overlays = await illustrator.analyze_text_placement(
                        image_data, scene_text, template=illustrator.template
                    )
                try:
                    ...
                    if text_overlays:
                        scene_update["text_overlays"] = text_overlays
                    async for doc_snap in scene_docs.where("scene_number", "==", scene_num).stream():
                        await doc_snap.reference.update(scene_update)
                    img_msg = {"type": "image", "content": cache_bust_url, "scene_number": scene_num, "tier": tier}
                    if text_overlays:
                        img_msg["text_overlays"] = text_overlays
                    await _safe_send(websocket, img_msg)
